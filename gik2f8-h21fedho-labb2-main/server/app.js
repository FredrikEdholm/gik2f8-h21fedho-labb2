...
/***********************Labb 2 ***********************/
/* Här skulle det vara lämpligt att skapa en funktion som, likt post eller delete, kan hantera PUT- eller PATCH-anrop (du får välja vilken metod som verkar mest lämplig för vad du ska göra) för att kunna markera uppgifter som färdiga. Den nya statusen, completed true eller false, kan skickas i förfrågans body (req.body) tillsammans med exempelvis id så att man kan hitta en specifik uppgift i listan, uppdatera uppgiftens status och till sist spara ner listan med den uppdaterade uppgiften. Observera att all kod relaterad till backend för labb 2 ska skrivas i den här filen och inte i app.node.js. App.node.js är bara till för exempel från lektion 5 och innehåller inte någon kod som används vidare under lektionerna.*/
...
/* Med app.listen startar man servern. Det första argumentet är portnumret som man vill att servern ska köra på, det är satt till 5000 på rad 9. Det andra argumentet är en anonym funktion som körs när servern har startat succesfullt. Ett meddelande skrivs ut för att ge feedback och bekräfta att allt har startat korrekt.
